# -*- coding: utf-8 -*-
"""
The colorizer module is a part of the Cloud2Craft project.

Colorizer is under MIT licence and copyright goes to
Antoine MIRAS and Baptiste BELLOCQ

"""


def read_palette_from_txt(palette_path):
    """
    read txt custom palette

    Parameters
    ----------
    palettePath : TYPE string
        file path to the palette txt.

    Returns
    -------
    palette_dict: TYPE dictionnary
        Dictionnary with color keys assigned to block id.
    keys: TYPE list
        Full color List.
    """
    try:
        palette_document = open(palette_path, "r")
        lines = palette_document.readlines()
        ...
        for line in lines:
            ...
            palette_dict[tuple(int(col) for col in colors.split(","))] = [
                int(blc) for blc in block.split(" ")]
        return palette_dict, list(palette_dict.keys())
...
def colorizer(palette_index, palette_path=""):
    """
    Return the color dictionnary for a given palette index

    Parameters
    ----------
    paletteIndex : TYPE integer
        index from the palette comboBox.
    palettePath : TYPE, optional
        Path if needed. The default is "".

    Returns
    -------
    TYPE dict
        Color dictionnary, with rgb in keys assigned to block id values.
    ...
    color_dict = {
        ...
        4: (read_palette_from_txt(palette_path)),
    }
    return color_dict[palette_index]
